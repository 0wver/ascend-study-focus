  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto overflow-x-hidden">
      <div className="flex min-h-screen items-center justify-center p-4 text-center">
        ...
        {/* Calendar Modal */}
        <div
          className="bg-white dark:bg-black/40 border border-gray-200 dark:border-white/15 w-full max-w-md overflow-hidden p-6 text-left shadow-xl relative z-10 transition-all duration-300 transform scale-100 opacity-100 backdrop-blur-md rounded-xl"
        >
          {/* Header */}
          <div className="mb-5 flex items-center justify-between">
            <h3 className="text-xl font-medium text-gray-800 dark:text-white flex items-center">
              <FiCalendar className="mr-2" />
              Select Date
            </h3>
            <button
              onClick={onClose}
              className="rounded-full p-1 text-gray-700 dark:text-white hover:bg-gray-100 dark:hover:bg-white/10 focus:outline-none"
            >
          ...
          <div className="flex justify-between items-center mb-4">
            <button 
              ...
              className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 text-gray-700 dark:text-white/80 focus:outline-none transition-colors duration-200"
            >
          ...
          <div className="grid grid-cols-7 mb-2">
            {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map((day) => (
              <div key={day} className="text-center text-sm text-gray-500 dark:text-white/60 font-medium py-2">
                {day}
              </div>
            ))}
          </div>
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${!isSameMonth(day, currentMonth) ? 'text-gray-300 dark:text-white/30' : 'text-gray-800 dark:text-white font-medium'}
                    ${isCurrentDay && !isSelected ? 'bg-primary-500/20 text-primary-800 dark:text-white font-semibold' : ''}
                    ${isSelected ? 'bg-primary-600 text-white shadow-[0_0_10px_rgba(249,115,22,0.4)] font-semibold' : ''}
                    ${!isSelected && !isCurrentDay ? 'hover:bg-gray-100 dark:hover:bg-white/10' : ''}
