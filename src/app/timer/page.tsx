...
export default function TimerPage() {
  ...
  // Filter study habits
  const studyHabits = useMemo(() => {
    // First get all parent habits that are study/work related and have duration
    const parentHabits = habits
      .filter(h => 
        (h.category === 'study' || h.category === 'work') 
        && h.duration
        && !h.isSubHabit
      )
      .sort((a, b) => {
        // Sort by progress - use a safer way to calculate progress
        // First check if each habit has completions today
        const today = new Date().toISOString().split('T')[0];
        
        // Calculate progress for habit A
        const aCompletedHours = a.completions
          .filter(c => c.date.split('T')[0] === today)
          .reduce((sum, c) => sum + c.count, 0);
        const aProgress = a.duration ? (aCompletedHours / a.duration) * 100 : 0;
        
        // Calculate progress for habit B
        const bCompletedHours = b.completions
          .filter(c => c.date.split('T')[0] === today)
          .reduce((sum, c) => sum + c.count, 0);
        const bProgress = b.duration ? (bCompletedHours / b.duration) * 100 : 0;
        
        // If one is significantly behind and the other is close to completion
        if (aProgress < 50 && bProgress > 80) return -1;
        if (bProgress < 50 && aProgress > 80) return 1;
        
        // Sort by overall completion percentage
        return aProgress - bProgress;
      });
    
    // Get all sub-habits with their parent names
    const subHabits = habits
      .filter(h => 
        (h.category === 'study' || h.category === 'work') 
        && h.duration
        && h.isSubHabit
      ...
      .map(subHabit => {
        // Find the parent habit to get its name
        const parentHabit = habits.find(h => h.id === subHabit.parentId);
        const parentName = parentHabit ? parentHabit.name : '';
        
        // Return the sub-habit with extra display information
        return {
          ...subHabit,
          displayName: `${parentName} - ${subHabit.name}`
        } as typeof subHabit & { displayName: string };
      })
      .sort((a, b) => a.displayName.localeCompare(b.displayName));
    
    // Combine parent habits and sub-habits
    return [...parentHabits, ...subHabits];
  }, [habits]);
  ...
  const handleStartStudySession = () => {
    ...
    // Determine the proper subject name based on whether it's a sub-habit
    let sessionSubject = selectedHabit.name;
    
    // If it's a sub-habit, prefix with parent name
    if (selectedHabit.isSubHabit && selectedHabit.parentId) {
      const parentHabit = habits.find(h => h.id === selectedHabit.parentId);
      if (parentHabit) {
        sessionSubject = `${parentHabit.name} - ${selectedHabit.name}`;
      }
    }
    ...
    const studySessionId = addStudySession({
      ...
      tags: [],
      // Add the habitId as a custom property that the store will handle
      startTime: new Date().toISOString(),
      // @ts-ignore - habitId is handled in the store implementation
        ...
        {activeHabitId && habitProgress && (
          <div className="mb-8 p-5 bg-gradient-to-br from-study-primary/20 to-study-primary/5 rounded-xl border border-study-primary/30 shadow-lg shadow-study-primary/10">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-lg font-medium text-white">
                {(() => {
                  ...
                  if (activeHabit?.isSubHabit && activeHabit.parentId) {
                    const parentHabit = habits.find(h => h.id === activeHabit.parentId);
                    if (parentHabit) {
                      return `${parentHabit.name} - ${activeHabit.name}`;
                    }
                  }
                  return activeHabit?.name || 'Study Progress';
                })()}
              </h3>
            ...
            <div className="w-full bg-black/40 rounded-full h-3 mb-2 overflow-hidden">
              <div 
                className="h-full rounded-full bg-study-primary transition-all duration-1000 ease-out"
                      ...
                      >
                        ...
                        {studyHabits.map(habit => (
                          <option key={habit.id} value={habit.id} style={{backgroundColor: '#333', color: 'white'}}>
                            {('displayName' in habit ? habit.displayName : habit.name) + (habit.duration ? ` (${habit.duration} hours)` : '')}
                          </option>
                        ))}
                      </select>
                        ...
                        <div className="mt-3 p-3 bg-study-primary/20 rounded-lg border border-study-primary/30">
                          <div className="flex justify-between items-center">
                            <p className="text-sm text-white font-medium">
                              {(() => {
                                const selectedHabit = habits.find(h => h.id === selectedHabitId);
                                if (selectedHabit?.isSubHabit) {
                                  const parentHabit = habits.find(h => h.id === selectedHabit.parentId);
                                  if (parentHabit) {
                                    return `${parentHabit.name} - ${selectedHabit.name}`;
                                  }
                                }
