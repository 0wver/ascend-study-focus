...
export default function StatsPage() {
  ...
  // Get processed data with week-by-week stats - moved before conditional return
  const statsData = useMemo(() => {
    // Only include parent habits, not sub-habits
    const parentHabits = habits.filter(habit => !habit.isSubHabit);
    
    return parentHabits.map(habit => {
      ...
      return {
        ...
        // ... any other stats you want to calculate
      };
    });
  }, [habits]);

  // Get habits sorted by category for the category breakdown
  const habitsByCategory = useMemo(() => {
    ...
    statsData.forEach(item => {
      const category = item.habit.category;
  ...
  const topHabits = useMemo(() => {
    ...
    return [...statsData]
      .sort((a, b) => {
        const aCompletions = Object.values(a.completionsByDate).reduce((sum, count) => sum + count, 0);
        const bCompletions = Object.values(b.completionsByDate).reduce((sum, count) => sum + count, 0);
  ...
  // Calculate weekly completion stats for charts
  const weeklyCompletionStats = useMemo(() => {
    if (selectedHabit) {
      const statsItem = statsData.find(item => item.habit.id === selectedHabit);
      if (!statsItem) return [];
      
      // ... calculation for weekly stats using the statsItem ...
    }
    return [];
  }, [selectedHabit, statsData]);
  
  // Calculate today's completed habits count, including parent habits with sub-habits
  const todayStr = format(new Date(), 'yyyy-MM-dd');
  const completedHabitsToday = useMemo(() => {
    return habits.filter(habit => {
      // Skip sub-habits since they're counted through their parents
      if (habit.isSubHabit) return false;
      ...
      if (habit.subHabits && habit.subHabits.length > 0) {
        ...
        // Check if ALL sub-habits are completed today
        const allSubHabitsComplete = subHabits.every(subHabit => {
          ...
          if ((subHabit.category === 'study' || subHabit.category === 'work') && subHabit.duration) {
            const completedHours = subHabit.completions
              .filter(c => c.date.split('T')[0] === todayStr)
          ...
          // For regular habits
            ...
            return subHabit.completions.some(c => c.date.split('T')[0] === todayStr);
          }
        });
      ...
      // For regular habits without duration
        ...
        return habit.completions.some(c => c.date.split('T')[0] === todayStr);
      }
    });
  }, [habits, todayStr]);
  ...
  const habitCompletionData = daysInMonth.map(day => {
    ...
    // Calculate completed parent habits for this day, properly handling sub-habits
    const completedHabits = habits.filter(habit => {
      ...
      if (habit.subHabits && habit.subHabits.length > 0) {
        ...
        // Check if ALL sub-habits are completed for this day
        const allSubHabitsComplete = subHabits.every(subHabit => {
          // For study/work habits with duration
          if ((subHabit.category === 'study' || subHabit.category === 'work') && subHabit.duration) {
            const completedHours = subHabit.completions
              .filter(c => c.date.split('T')[0] === dayStr)
              .reduce((sum, c) => sum + c.count, 0);
          ...
          // For regular habits
            ...
            return subHabit.completions.some(c => c.date.split('T')[0] === dayStr);
          }
        });
      ...
      // For regular habits without duration
        ...
        return habit.completions.some(c => c.date.split('T')[0] === dayStr);
      }
    }).length;
    ...
    return {
      ...
      total: habits.filter(h => !h.isSubHabit).length,
    };
  });
  ...
  const getDaysActiveForHabit = (habit: any) => {
    ...
    <>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          <div className="glow-card teal p-4 rounded-xl bg-gradient-to-br from-teal-600/30 to-teal-400/10 border border-teal-500/30 hover:-translate-y-1 transition-transform duration-300">
            <div className="flex items-center">
              <div className="p-2 rounded-full bg-teal-500/40 text-white mr-3 shadow-[0_0_10px_rgba(20,184,166,0.4)]">
                <FiCheck className="w-5 h-5" />
              </div>
              <div>
                <p className="text-sm text-white/80">Completed Today</p>
                <p className="text-xl font-bold text-white">
                  {completedHabitsToday.length}/{habits.filter(h => !h.isSubHabit).length}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="glow-card purple p-4 rounded-xl bg-gradient-to-br from-purple-600/30 to-purple-400/10 border border-purple-500/30 hover:-translate-y-1 transition-transform duration-300">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-white">{habits.filter(h => !h.isSubHabit).length}</p>
              </div>
            </div>
          </div>
